...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("climacoin"))
        return false;
...
{
    // Convert climacoin:// to climacoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("climacoin://"))
    {
        uri.replace(0, 12, "climacoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "climacoin.lnk";
}
...
{
    return GetAutostartDir() / "climacoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("climacoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  climacoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("climacoin-Qt"));
